...
@app.route('/index')
def index():
    if 'conectado' in session:  # Verifica si el usuario está conectado o autenticado como lo haces en otras partes del código
...
def mapa():
    if 'conectado' in session:  # Verifica si el usuario está conectado o autenticado como lo haces en otras partes del código
        return render_template('public/mapas-sonoros.html')
    else:
        flash('Primero debes iniciar sesión.', 'error')
        return redirect(url_for('inicio'))
